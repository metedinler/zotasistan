...
# ----------------------------
LOG_DOSYASI = os.path.join(KAYNAK_DIZIN, "islem_logu.json" if KAYNAK_DIZIN else Path("islem_logu.json"))    # islem_logu.json dosyası
...
# REMARK: Yukarıdaki response kontrolü kodu kaldırıldı çünkü global alanda istek yapılıp kontrol edilmeye çalışılmıştı.
...
def init_dirs():
    ...
    for d in [HEDEF_DIZIN, TEMIZ_TABLO_DIZIN, TEMIZ_KAYNAKCA_DIZIN, str(PDF_DIR), str(EMBEDDING_PARCA_DIR)]:
...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        logger.error(f"❌ PDF metni çıkarılamadı: {pdf_path}, Hata: {e}")  # REMARK: Hata mesajı düzenlendi.
        traceback.print_exc()
        return None
...
def extract_references_enhanced(text):
    ...
    """
    references = []
    # REMARK: İki farklı pattern ekledik. İlk pattern kaynakça başlığı sonrası tüm satırları alır,
    # ikinci pattern ise [1] formatındaki referansları yakalar.
    ref_patterns = [
    ...
    # REMARK: Temizlik ve kısa referans kontrolü yapılıyor.
    cleaned_refs = []
...
def save_references(references, file_path, source_type="pdf"):
    ...
    # JSON formatında kaydetmek için dosya adı belirleniyor.
...
# ----------------------------
# REMARK: get_embedding fonksiyonunu kaldırdık ve embedding işlemini embed_text fonksiyonunda topladık.
def embed_text(text):
